...
```bash
docker compose up -d   # Keycloak on localhost:8081
```
...
- `config/SecurityConfiguration.kt` — Two filter chains: actuator chain (permit-all for `/.well-known/**`, `/error`) and MCP chain (requires `SCOPE_mcp:tools` on `/mcp/**`). OAuth2 resource server trusts Keycloak at `http://localhost:8081/realms/master`.
